...
We can of course write ourselves some simple functions in python to calculate this for us.
...
r = correlation_coefficient(ls_x, ls_y)
r_squared = round(r**2, 2)*100

print('Y = {}x + {}'.format(ls_slope, ls_intercept))
print('r = {} & r squared = {}%'.format(r, r_squared))
```
![](/jupyter-notebooks/least-square-example.png)

Our $r$ value indicates a strong negative correlation between our variables.  We observe the $r^2$ value of $0.89$ which represents the variation in $y$ that is explained by the linear relationship between $x$ and $y$. This means that approximately $89.0\%$ of the variation in $y$ is explained by the linear relationship. Even though we have a strong linear correlation, this does not always imply causation.  There are further techniques that can be applied that are outside the scope of this post.
...
The Echelon method can be thought of as an elimination method.  The elimination method allows for any variable to be removed. Once we're able to eliminate a variable, we can use *back substitution* to calculate the remaining variable(s). Let's look at a simplified example of this:
...
In addition to these two methods we also covered matrix addition, subtraction, & multiplication. We also reviewed how to calculate the inverse of a matrix. In order to keep this post relatively concise, I'm not going to cover each of those topics.

##### The Simplex Method

The final section that I'm going to cover regarding **Linear Programming** is the *Simplex Method*. The simplex method arose from a practice problem faced by George B. Dantzip in 1947. Dantzig was concerned with minimizing the the cost of allocating supplies to the United States Air Force \[1\]. The prior chapter in the book dealt with finding the **feasible region** via the graphical method. The graphical method is really only practical when there are two variables.
